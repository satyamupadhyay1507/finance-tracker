...
async function generate() {
  ...
  let sql = `-- Seed data for Finance Tracker\n\n`;
  ...
  const users = [
    ...
    { id: 3, name: 'Read Only User', email: 'readonly@demo.com', role: 'read-only' }
  ];

  for (const u of users) {
    sql += `INSERT INTO users (id, name, email, password, role) VALUES (${u.id}, '${u.name}', '${u.email}', '${hashedPassword}', '${u.role}') ON DUPLICATE KEY UPDATE id=id;\n`;
  }
  sql += `\n`;

  // Categories are already inserted by schema.sql, but we need their IDs.
  // We'll just assume the standard IDs based on schema.sql insertion order.
  const catMap = {
    'Food': 1, 'Transport': 2, 'Entertainment': 3, 'Shopping': 4,
    'Bills': 5, 'Health': 6, 'Education': 7, 'Salary': 8,
    'Freelance': 9, 'Investment': 10, 'Other': 11
  };
  ...
  for (const email of ['admin@demo.com', 'user@demo.com', 'readonly@demo.com']) {
    ...
    for (const t of sampleTransactions) {
      ...
      sql += `INSERT INTO transactions (user_id, type, amount, category_id, description, date) VALUES (${userId}, '${t.type}', ${t.amount}, ${catId}, '${t.description}', '${dateStr}');\n`;
    }
  }

  fs.writeFileSync('../database/seed_data.sql', sql);
  console.log('Generated ../database/seed_data.sql');
}
